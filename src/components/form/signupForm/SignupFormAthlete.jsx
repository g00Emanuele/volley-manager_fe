...
const SignupForm = () => {
  ...
  const uploadFile = async (cover) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BASE_URL}/athletes/cloudUpload`, {
        method: "POST",
        body: fileData,
      });
  ...
  return (
    <>
      <Nav.Link onClick={handleShow}>Subscribe as athlete</Nav.Link>

      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Nuovo atleta</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form 
          ...
          <Form.Group className="mb-3">
            <Form.Select name="role" onChange={handleInputChange}>
              ...
              <option value="middle-blocker">Centrale</option>
              <option value="outside-hitter">Banda</option>
              <option value="opposite-hitter">Opposto</option>
              <option value="libero">Libero</option>
            </Form.Select>
          </Form.Group>

          <Form.Group className="mb-3">
            <Form.Label>Scegli la squadra in cui vorresti entrare</Form.Label>
